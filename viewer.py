...
datasource = ColumnDataSource({"x": [], "y": [], "dx": [], "dy": [], "name": [], "size": [], "location": []})
...
def generate_treemap_data():
    ...
    column_data["location"] = [x["location"] for x in treemap_data]
...
def add_if_leaf(node: parser.Variable, depth: int, input_square: dict[int, int, int, int], path_name: str):
    ...
    if depth == 0 or (not isinstance(node.type, parser.Struct) and not isinstance(node.type, parser.Array)):
        ...
        if isinstance(node.type, parser.Struct):
            ...
            children = [
                parser.Variable(tag="", die_offset=0, name=f"[{i}]", location=0, type=node.type.array_elements)
                for i in range(node.type.array_size)
            ]
...
def build_overview_text_recursive(variable, depth):
    html = f"<li id='{variable.location:08x}'>"
    ...
    if depth > 0:
        if isinstance(variable.type, parser.Struct):
            html += "<ul>"
            for member in sorted(variable.type.members, key=lambda x: x.type.size, reverse=True):
                html += build_overview_text_recursive(member, depth - 1)
            html += "</ul>"
        ...
        if isinstance(variable.type, parser.Array):
            ...
            fake_members = [
                parser.Variable(
                    ...
                    die_offset=0,
                    name=f"[{i}]",
                    location=i * variable.type.array_elements.size,
                    type=variable.type.array_elements,
                )
                for i in range(variable.type.array_size)
            ]
...
def build_overview_text(depth: int = 10):
    html = "<ul>"
    for variable in sorted(variables, key=lambda x: x.type.size, reverse=True):
        html += build_overview_text_recursive(variable, depth - 1)
    html += "</ul>"
    return html
...
def upload_elf(attr, old, new):
    ...
    global variables
    variables = parser.process_elffile(elffile)

    status_text.text = "Building map"
    generate_treemap_data()
    overview.text = build_overview_text(max_depth)
...
treemap.block(
    "x", "y", "dx", "dy", source=datasource, line_width=1, line_color="white", fill_alpha=0.8
)  # , fill_color=factor_cmap("name", "MediumContrast4", datasource))

status_text = Div(text="pick a file")

overview = Div(text="pick a file")

layout = layout([[depth_slider], [file_input], [status_text], [treemap], [overview]])
